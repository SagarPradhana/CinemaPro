...
const genres = ['All', 'Action', 'Drama', 'Comedy', 'Horror', 'Sci-Fi', 'Romance', 'Thriller', 'Animation', 'Adventure'];

export default function Movies() {
  const dispatch = useAppDispatch();
  const { items, loading, pagination } = useAppSelector((state) => state.movies);
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          >
            <div className="w-16 h-16 rounded-3xl bg-[#B8892A]/10 flex items-center justify-center shadow-sm">
              <Film className="h-8 w-8 text-[#B8892A]" />
            </div>
            <div>
              <h1 className="text-5xl lg:text-6xl font-display font-black text-[#1A1510] tracking-tighter uppercase leading-none">Film Vault</h1>
              <p className="text-[#8A7A65] font-medium mt-2 tracking-[0.1em] uppercase text-[11px] font-black">Discover the latest cinematic blockbusters</p>
            </div>
          </motion.div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        {loading ? (
          ...
          <div className="py-40 text-center">
            <Film className="mx-auto h-24 w-24 text-[#B8892A]/10" />
            <h3 className="mt-8 text-3xl font-display font-black text-[#1A1510]">No features found</h3>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-10 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5">
              {items.map((movie) => (
                <ContentCard key={movie._id} data={movie} type="movie" />
              ))}
            </div>
