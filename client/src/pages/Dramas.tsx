...
import { fetchDramas } from '@/store/seriesSlice';
import { debounce } from '@/lib/utils';

const genres = ['All', 'Action', 'Drama', 'Comedy', 'Sci-Fi', 'Romance', 'Thriller', 'Fantasy', 'Mystery'];

export default function Dramas() {
  const dispatch = useAppDispatch();
  const { items, loading, pagination } = useAppSelector((state) => state.series);
  ...
  useEffect(() => {
    ...
    dispatch(fetchDramas(params));
  }, [dispatch, selectedGenre, search, searchParams.get('page')]);

  const handleSearch = debounce((value: string) => {
    setSearch(value);
    if (value) setSearchParams({ search: value, genre: selectedGenre });
    else setSearchParams({ genre: selectedGenre });
  }, 300);

  const handleGenreClick = (genre: string) => {
    if (genre === 'All') setSearchParams(search ? { search } : {});
    else setSearchParams({ genre, ...(search ? { search } : {}) });
  };
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          >
            <div className="w-16 h-16 rounded-3xl bg-[#B8892A]/10 flex items-center justify-center shadow-sm">
              <Clapperboard className="h-8 w-8 text-[#B8892A]" />
            </div>
            <div>
              <h1 className="text-5xl lg:text-6xl font-display font-black text-[#1A1510] tracking-tighter uppercase leading-none">Drama Theatre</h1>
              <p className="text-[#8A7A65] font-medium mt-2 tracking-[0.1em] uppercase text-[11px] font-black">Captivating Asian dramas and stories</p>
            </div>
          </motion.div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        {loading ? (
          ...
          <div className="py-40 text-center">
            <Clapperboard className="mx-auto h-24 w-24 text-[#B8892A]/10" />
            <h3 className="mt-8 text-3xl font-display font-black text-[#1A1510]">No dramas found</h3>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-10 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5">
              {items.map((drama) => (
                <ContentCard key={drama._id} data={drama} type="drama" />
              ))}
            </div>
            {totalPages > 1 && (
